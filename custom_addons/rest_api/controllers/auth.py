...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/auth/get_tokens', methods=['POST'], type='http', auth='none', csrf=False, cors='*')
    def api_auth_gettokens(self):
        ...
        return werkzeug.wrappers.Response(
            ...
            response = json.dumps({
                ...
                'response':200,
                'type':request.env.user.x_typeofcontact,
                'sale_team_id':request.env.user.sale_team_id.id,
                'partner_id': request.env.user.partner_id.id if uid else 'null'}),
        )


    # Refresh access token:
    @http.route('/api/auth/refresh_token', methods=['POST'], type='http', auth='none', csrf=False, cors='*')
    ...
    @http.route('/api/auth/delete_tokens', methods=['POST'], type='http', auth='none', csrf=False, cors='*')
