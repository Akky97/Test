...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/moashk/happy_birthday', method=['POST'], type='http', auth='none', csrf=False, cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute(
                    "select name,email,to_char(x_dob, 'DD-MM') as dob from res_partner where to_char(NOW(), 'DD-MM') = to_char(x_dob, 'DD-MM')")
                r = (cr.dictfetchall())
                for i in r:
                    email = i['email']
                    name = i['name']
                    ...
                    for i in range(len(li)):
                        ...
                        body = """ <html>
                            ...
                            name) + """</p></b>
                                                                                                                     <br>
                                                                                                                     <b><p> Team Moashk wishes you many happy returns of the day.</p><b>
