...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/lastyear', method=['POST'], type='http', auth='none', csrf=False ,cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute("SELECT  TO_CHAR(t1.create_date,'Mon') AS month,t1.user_type_id,SUM(t1.debit) as debit ,SUM(t1.credit) as credit FROM account_move_line AS t1 LEFT JOIN account_account AS t2 ON t1.account_id = t2.id WHERE t1.company_id = '" + str(company_id) + "' AND t1.create_date > date_trunc('month', CURRENT_DATE) - INTERVAL '1 year' AND (t1.user_type_id = 16 OR t1.user_type_id = 14) GROUP BY TO_CHAR(t1.create_date,'Mon'),t1.user_type_id ORDER BY  TO_CHAR(t1.create_date,'Mon')")
                temp = []
                rows = cr.fetchall()
