...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/trialbalance', method=['POST'], type='http', auth='none', csrf=False ,cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute("SELECT  EXTRACT(MONTH FROM t1.create_date) AS mon,t1.user_type_id,t2.name, t1.account_id,SUM(t1.debit)as debit,SUM(t1.credit)as credit FROM account_move_line AS t1 LEFT JOIN account_account AS t2 ON t1.account_id = t2.id WHERE t1.company_id = '" + str(company_id) + "' AND ( ( EXTRACT(MONTH FROM t1.create_date) = " + str(month) + " AND EXTRACT(YEAR FROM t1.create_date) = " + str(year) + " ) OR ( EXTRACT(MONTH FROM t1.create_date) = " +str(lastmonth)+" AND EXTRACT(YEAR FROM t1.create_date) = " +str(lastyear)+" ) ) GROUP BY EXTRACT(MONTH FROM t1.create_date),t1.user_type_id,t2.name,t1.account_id ORDER BY  EXTRACT(MONTH FROM t1.create_date),t2.name,t1.account_id")
                temp = []
                rows = cr.fetchall()
                print('Total Row(s):', rows)
                for row in rows:
                    obj = {"month":row[0],"user_type_id":row[1],"name":row[2],"account_id":row[3],"debit":row[4],"credit":row[5]}
                    temp.append(obj)
