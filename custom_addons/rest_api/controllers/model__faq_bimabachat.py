...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/faqweb', method=['POST'], type='http', auth='none', csrf=False ,cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                ...
                cr.execute("select t2.name as infotype,t3.name as infodata,t4.name as infosubdata,t1.question,t1.answer,t1.sequence from webpage1_webpage1 as t1 left join infotype_infotype as t2 on t1.infotype = t2.id left join infosubdatadropdown as t3 on t1.infodata = t3.id left join subdata_subdata as t4 on t1.infosubtype = t4.id where t3.name = '" + str(infodata) + "' order by t1.sequence")
                temp = []
                rows = cr.fetchall()
                print('Total Row(s):', rows)
                for row in rows:
                    obj = {"infotype":row[0],"infodata":row[1],"infosubdata":row[2],"question":row[3],"answer":row[4],"sequence":row[5]}
                    temp.append(obj)
