...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/bimabachat/overdue_report', method=['POST'], type='http', auth='none', csrf=False, cors='*')
        ...
        if not db_name:
            ...
            query="select distinct(t2.id),t3.name as name,t2.login from crm_lead as t1  \
                        ...
                        left join res_partner as t3 on t2.partner_id =t3.id \
                         where (to_date(to_char(t1.date_action,'yyyy-mm-dd'),'yyyy-mm-dd')) <  (to_date(to_char(Now(), 'yyyy-mm-dd'), 'yyyy-mm-dd')) and not (t1.stage_id =27 or t1.stage_id =4) and t2.login  is not null and t1.active =True  \
            ...
            with registry.cursor() as cr:
                ...
                all = json.loads(auth)
                for id in all:
                    cr.execute("select t2.id,t3.name as salesperson,t1.partner_name,t2.login ,t1.date_action,t1.name,t5.name as companyname, \
                      ...
                      where (to_date(to_char(t1.date_action,'yyyy-mm-dd'),'yyyy-mm-dd')) <  (to_date(to_char(Now(), 'yyyy-mm-dd'), 'yyyy-mm-dd')) and not (t1.stage_id =27 or t1.stage_id =4) and t1.active=True  and t1.user_id = '" + str(id['id']) + "' \
                    ...
                    # msg['cc'] = cc
                    print (msg['To'], "TO_msg")
                    msg['Subject'] = "OVERDUE NOTIFICATION"
                    # rcpt = li + ',' + cc
                    rcpt = li
                    print (msg, "msg")
                    body = """ <html>
