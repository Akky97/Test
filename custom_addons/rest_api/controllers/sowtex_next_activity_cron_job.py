...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/next_activity_corn', method=['POST'], type='http', auth='none', csrf=False, cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            query="select t2.id,t3.name,t2.login ,t1.date_action,t1.name  from crm_lead as t1  left join res_users as t2  on t1.user_id =t2.id  \
                ...
                for id in all:
                    ...
                    if id['id'] == None:
                        ...
                        cr.execute("select t2.id,t3.name,t2.login ,t1.date_action,t1.name \
                                    ...
                                    left join res_partner as t3 on t2.partner_id =t3.id \
                                    where to_char(t1.date_action,'yyyy-mm-dd') = to_char(NOW()+interval '1 day','yyyy-mm-dd') and t1.user_id ='" + str(id['id']) + "' \
                                    group by t2.login,t3.name , t1.date_action,t1.name,t2.id order by t3.name ")
                        r1 = (cr.dictfetchall())
                        auth = json.dumps(r1)
                        all = json.loads(auth)
                        ...
                        for id1 in all:
                            ...
                            tbl += "<tr><td style='border: 1px solid black'>" + id1[
                    ...
                    for i in range(len(li)):
                        ...
                        body = """ <html>
                                             <head></head>
                                                                                     ...
                                                                                     <p>Hi """ + str(id['name']) + """</p>
                                                                                     <br>
                                                                                     <p> You have received your next_activity's Tickets.</p>
                                                                                     <br>
                                                                                     <br>
                                                                                     <p>Please login into your sowtex.mykumpany.com account to check the Pipeline next_activity's details.</p>
                                                                                     <br>
                                                                                     """ + tbl + """
                                                                                     <br>
                                                                                     <br>
