...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/moashk/overdue_report_sales_manager', method=['POST'], type='http', auth='none', csrf=False, cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute("Select t1.name,t3.name,t2.sale_team_id,t2.login from crm_team as t1 left join res_users as t2 on t1.user_id = t2.id left join res_partner as t3 on t2.partner_id = t3.id")
                r = (cr.dictfetchall())
                temp = []
                auth = json.dumps(r)
                all = json.loads(auth)
                for id in all:
                    ...
                    username = 'moashk@mykumpany.com'
                    password = 'moashk@2020'
                    try:
                        s = smtplib.SMTP('mail.mykumpany.com', 587)
                        s.starttls()
                        s.login(username, password)
                        # li = id['login']
                        li = "deepak@mykumpany.com"
                        cc = ""
                        msg = MIMEMultipart('alternative')
                        msg['from'] = "bharat@moashkinvestment.com"
                        msg['To'] = li
                        msg['cc'] = cc
                        print (msg['To'], "TO_msg")
