...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/customerexp', method=['POST'], type='http', auth='none', csrf=False ,cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute("SELECT t1.amount,t1.name as bill,t2.name,t1.state from account_move as t1 left join res_partner as t2 on t1.partner_id = t2.id where t1.name like 'INV%' and t1.company_id = '" + str(company_id) + "' and EXTRACT(MONTH FROM t1.create_date) = " + str(month) + "")
                temp = []
                rows = cr.fetchall()
                print('Total Row(s):', rows)
                for row in rows:
                    obj = {"amount":row[0],"bill":row[1],"name":row[2],"state":row[3]}
                    temp.append(obj)
