...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/apis', method=['GET'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
    def api_top_agent(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute("select name, key, value , type,page from apis_apis")
                rows = cr.fetchall()
                data = []
                for row in rows:
                    obj={
                    ...
                    data.append(obj)  # making array to append
