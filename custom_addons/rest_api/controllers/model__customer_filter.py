...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/customer_apis', method=['GET'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
    def api_top_agent(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                try:
                    cr.execute("select name, key, value , type,page from customer_filter_customer_filter")
                    rows = cr.fetchall()
                    data = []
                    for row in rows:
                        obj={
