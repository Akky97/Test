...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/moashk/next_activity_cron_job', method=['POST'], type='http', auth='none', csrf=False,
                cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            query = "select distinct(t2.id),t3.name,t2.login  from crm_lead as t1  left join res_users as t2  on t1.user_id =t2.id \
                    left join res_partner as t3 on t2.partner_id =t3.id \
                    left join crm_stage as t4 on t1.stage_id =t4.id \
                    where to_char(t1.date_action,'yyyy-mm-dd') = to_char(NOW()+interval '1 day','yyyy-mm-dd') and t2.login is not null \
                    and t4.name NOT IN ('Close (Won)','Open (Lost)') \
            ...
            with registry.cursor() as cr:
                ...
                all = json.loads(auth)
                for id in all:
                    cr.execute("select t2.id,t3.name as username,t2.login ,t1.date_action,t1.name,t1.id  as pipelineid \
                                                       from crm_lead as t1 \
                                                       left join res_users as t2  on t1.user_id =t2.id \
                                                       left join res_partner as t3 on t2.partner_id =t3.id \
                                                       left join crm_stage as t4 on t1.stage_id =t4.id \
                                                       where to_char(t1.date_action,'yyyy-mm-dd') = to_char(NOW()+interval '1 day','yyyy-mm-dd') and t4.name NOT IN ('Close (Won)','Open (Lost)') and t1.user_id ='" + str(id['id']) + "' \
                    ...
                    msg['cc'] = cc
                    print (msg['To'], "TO_msg")
                    msg['Subject'] = "Activity:" + str(id1['date_action']) + ""
                    rcpt = li + ',' + cc
                    print (msg, "msg")
                    body = """ <html>
                                                                                                                                                                                                                           <head></head>
                        ...
                        id1['username']) + """</p>
                                                                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                                                                   <br>
                                                                                                                                                                                                                                                                   <br>
                                                                                                                                                                                                                                                                   <p>Please login into your moashk.mykumpany.com account to check the Pipeline Next Activity's details.</p>
                                                                                                                                                                                                                                                                   <br>
                                                                                                                                                                                                                                                                   """ + tbl + """
                                                                                                                                                                                                                                                                   <br>
                                                                                                                                                                                                                                                                   <br>
