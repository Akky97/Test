...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/bimabachat/categorycount', method=['POST'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
    def api_top_count(self):
        ...
        if not db_name:
            ...
            with registry.cursor() as cr:
                cr.execute("select count(t2.name),t1.name as categoryname \
                            from category_category as t1  \
                            left join crm_lead as t2 on t1.id = t2.x_categoryone where t2.user_id = '" + str(user_id) + "'  group by t1.name having count(t2.name) >0")
