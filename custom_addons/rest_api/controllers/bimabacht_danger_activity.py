...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/bimabachat/next_activity_cron_job_danger', method=['POST'], type='http', auth='none', csrf=False, cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            query="select distinct(t2.id),t3.name as name,t2.login from crm_lead as t1  left join res_users as t2  on t1.user_id =t2.id  \
                            ...
                            and (to_date(to_char(Now(), 'yyyy-mm-dd'), 'yyyy-mm-dd') + interval '2 day') and not (t1.stage_id =27 or t1.stage_id =4)  and t2.login is not null\
                            and t1.active =True and t1.date_action is not null group by t2.id,t3.name order by t3.name"
            with registry.cursor() as cr:
                ...
                email =''
                for id in all:
                    cr.execute("select t2.id,t1.partner_name,t3.name as salesperson,t2.login ,t1.date_action,t1.name,t5.name as companyname, \
t1.id  as pipelineid,t4.name as subcategory ,t7.name as nextactivity,t8.name as stage,t1.date_deadline as deadline,t1.planned_revenue as plannedrevenue \
                     from crm_lead as t1 \
                      ...
                      where t1.date_deadline  BETWEEN (to_date(to_char(NOW(),'yyyy-mm-dd'),'yyyy-mm-dd'))\
                      and (to_date(to_char(Now(), 'yyyy-mm-dd'), 'yyyy-mm-dd') + interval '2 day') and not (t1.stage_id =27 or t1.stage_id =4) and t1.user_id = '" + str(id['id']) + "' \
                    ...
                    tbl = "<table style='border: 1px solid black'>" \
                          ...
                          "<td style='border: 1px solid black'>Expected Revenue</td></tr>"
                    for id1 in alldata:
                        if not id1['login']:
                            pass
                        email=id1['login']
                    ...
                    if not email:
                        ...
                        msg['Subject'] = "Danger Notification"
                        # rcpt = cc.split(",") + [li]
                        rcpt = [li]
                        print (msg, "msg")
                        body = """ <html>
