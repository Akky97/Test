...
# /api/state                GET     - Read all (with optional filters, offset, limit, order)
# /api/state/<id>           GET     - Read one
# /api/state                POST    - Create one
# /api/state/<id>           PUT     - Update one
# /api/state/<id>           DELETE  - Delete one
# /api/state/<id>/<method>  PUT     - Call method (with optional parameters)
...
#           ...
# OUT data:
OUT__state_state__SUCCESS_CODE = 200  # editable
...
class ControllerREST(http.Controller):
    ...
    # Read one:
    @http.route('/api/state/<id>', methods=['GET'], type='http', auth='none', cors='*')
    @check_permissions
    ...
    # Create one:
    @http.route('/api/state', methods=['POST'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    ...
    # Update one:
    @http.route('/api/state/<id>', methods=['PUT'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    ...
    # Delete one:
    @http.route('/api/state/<id>', methods=['DELETE'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    ...
    @http.route('/api/state/<id>/<method>', methods=['PUT'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    def api__project_defect__id__method_PUT(self, id, method):
        return wrap__resource__call_method(
            ...
            id=id,
            method=method,
            success_code=OUT__state_state__SUCCESS_CODE
        )
