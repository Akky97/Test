...
# /api/city                GET     - Read all (with optional filters, offset, limit, order)
# /api/city/<id>           GET     - Read one
# /api/city                POST    - Create one
# /api/city/<id>           PUT     - Update one
# /api/city/<id>           DELETE  - Delete one
# /api/city/<id>/<method>  PUT     - Call method (with optional parameters)
...
#           ...
# OUT data:
OUT__city_city__SUCCESS_CODE = 200  # editable
...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/city', methods=['GET'], type='http', auth='none', cors='*')
    @check_permissions
    ...
    # Read one:
    @http.route('/api/city/<id>', methods=['GET'], type='http', auth='none', cors='*')
    @check_permissions
    ...
    # Create one:
    @http.route('/api/city', methods=['POST'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    ...
    # Update one:
    @http.route('/api/city/<id>', methods=['PUT'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    ...
    # Delete one:
    @http.route('/api/city/<id>', methods=['DELETE'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    ...
    @http.route('/api/city/<id>/<method>', methods=['PUT'], type='http', auth='none', csrf=False, cors='*')
    @check_permissions
    def api__project_defect__id__method_PUT(self, id, method):
        return wrap__resource__call_method(
            ...
            id=id,
            method=method,
            success_code=OUT__city_city__SUCCESS_CODE
        )
