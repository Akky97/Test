...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/res.partner.count', methods=['GET'], type='http', auth='none' ,cors='*')
    @check_permissions
    def api__res_partner__GET(self, **kw):
        uid = request.session.uid
        datas = request.env['res.users'].sudo().search([('id', '=', uid)], limit=1)
        data123 = request.env['res.partner'].sudo().search(
            ['|', ('id', '=', datas.partner_id.id), ('create_uid', '=', uid)]).ids
    ...
    # Read one:
    @http.route('/api/res.partner.count/<id>', methods=['GET'], type='http', auth='none' ,cors='*')
    @check_permissions
    ...
    # Create one:
    @http.route('/api/res.partner.count', methods=['POST'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
    ...
    # Update one:
    @http.route('/api/res.partner.count/<id>', methods=['PUT'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
    ...
    # Delete one:
    @http.route('/api/res.partner.count/<id>', methods=['DELETE'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
    ...
    @http.route('/api/res.partner.count/<id>/<method>', methods=['PUT'], type='http', auth='none', csrf=False ,cors='*')
    @check_permissions
