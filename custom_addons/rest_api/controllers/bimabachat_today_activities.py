...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/bimabachat/next_activity_today_activity', method=['POST'], type='http', auth='none', csrf=False, cors='*')
    def api_top_count(self):
        ...
        if not db_name:
            ...
            query="select distinct(t2.id),t3.name as name,t2.login from crm_lead as t1  \
                        ...
                        where (to_date(to_char(t1.date_action, 'yyyy-mm-dd'), 'yyyy-mm-dd')) = (to_date(to_char(Now(), 'yyyy-mm-dd'), 'yyyy-mm-dd')) and not (t1.stage_id =27 or t1.stage_id =4) and t1.active =True and t2.login is not null \
            ...
            with registry.cursor() as cr:
                ...
                for id in all:
                    ...
                    # msg['cc'] = cc
                    print (msg['To'], "TO_msg")
                    msg['Subject'] = "TODAY NOTIFICATION"
                    rcpt = [li]
                    # rcpt = li + ',' + cc
                    print (msg, "msg")
                    body = """ <html>
