...
class ControllerREST(http.Controller):
    ...
    @http.route('/api/bimabachat/next_activity_cron_job_warning', method=['POST'], type='http', auth='none', csrf=False, cors='*')
        ...
        if not db_name:
            ...
            query="select distinct(t2.id),t3.name as name,t2.login from crm_lead as t1  left join res_users as t2  on t1.user_id =t2.id  \
                            left join res_partner as t3 on t2.partner_id =t3.id \
                             ...
                             and not (t1.stage_id =27 or t1.stage_id =4)  and t2.login is not null and t1.active =True \
                            group by t2.id,t3.name,t2.login order by t3.name,t2.login"
            with registry.cursor() as cr:
                ...
                print all,"ALLLSSS"
                for id in all:
                    cr.execute("select t2.id,t3.name as salesperson,t2.login ,t1.date_action,t1.name,t5.name as companyname, \
                      ...
                      where t1.date_deadline  BETWEEN (to_date(to_char(NOW(),'yyyy-mm-dd'),'yyyy-mm-dd') +interval '3 day') \
                      and (to_date(to_char(Now(), 'yyyy-mm-dd'), 'yyyy-mm-dd') + interval '5 day') \
                      and not (t1.stage_id =27 or t1.stage_id =4) and t1.user_id = '" + str(id['id']) + "' \
                      group by t2.login, t3.name, t1.date_action, t1.name, t2.id, t1.id, t4.name, t5.name, t7.name, t8.name, t1.date_deadline, t1.planned_revenue \
                      order  by  t3.name ")
                    r1 = (cr.dictfetchall())
                    auth = json.dumps(r1)
                    ...
                    # msg['cc'] = cc
                    # print (msg['To'], "TO_msg")
                    msg['Subject'] = "WARNING NOTIFICATION"
                    # rcpt =  cc.split(",") + [li]
                    rcpt =  [li]
                    print (msg, "msg")
                    body = """ <html>
