...
class tp_premium(models.Model):
    ...
    def _compute_date(self):
        if self.subcategory:
            date = {}
            temp=[]
            iname = self.env['tppremium'].search([('category', '=', self.category.id),
    ...
    def _compute_data(self):
        for i in self:
            data = self.env['tppremium'].search([('insurername','=',i.insurername.id),('category', '=', i.category.id),
            ...
            if number >1:
                ...
                if len(ids) == 1:
                    ...
                    for k in ids:
                        if k < largestnumber:
                            data = self.env['tppremium'].search([('id','=',k)])
                            print ("2")
        ...
        if self.fdate:
            if self.fdate >= self.tdate:
                ...
                if iname:
                    ...
                    vals['rate'] = self.rate
        return super(tp.p, self).write(vals)
    ...
    def create(self, vals):
        temp = []
        iname = self.env['tppremium'].search([('category', '=', vals.get('category')),
        ...
        return super(tp_premium, self).create(vals)
