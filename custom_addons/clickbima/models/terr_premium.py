...
class terr_premium(models.Model):
    _name = 'terrpremium'
    ...
    def _compute_date(self):
        if self.subcategory:
            date = {}
            temp=[]
            iname = self.env['terrpremium'].search([('category', '=', self.category.id),
    ...
    def _compute_data(self):
        for i in self:
            data = self.env['terrpremium'].search([('insurername','=',i.insurername.id),('category', '=', i.category.id),
            ...
            if number >1:
                ...
                if len(ids) == 1:
                    ...
                    for k in ids:
                        if k < largestnumber:
                            data = self.env['terrpremium'].search([('id','=',k)])
                            print ("2")
        ...
        if self.fdate:
            if self.fdate >= self.tdate:
                ...
                if iname:
                    ...
                    vals['rate'] = self.rate
        return super(terr_premium, self).write(vals)
    ...
    def create(self, vals):
        temp = []
        iname = self.env['terrpremium'].search([('category', '=', vals.get('category')),
        ...
        return super(terr_premium, self).create(vals)
