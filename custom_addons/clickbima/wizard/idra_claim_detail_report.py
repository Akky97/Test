...
class Idradetailreport(models.TransientModel):
    _name = "irdaclaimsdetail.report"
    _description = "IRDA Claim Detail Report"
    ...
    filterby = fields.Selection([('all', 'All'),('false', 'Individual'), ('true', 'Company'),('individual_client','Individual Client'),('individual_group','Individual Group'),('internal_reference','Internal Reference')] , default='all', string="Filter BY")
    ...
    def generate_xlsx_report(self):
        ...
        return self.env['report'].get_action(self, report_name='clickbima.irda_claims_detail.xlsx', data=data)
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        with registry.cursor() as cr:
            ...
            if lines.fiscal_year and lines.monthly == "yearly" and not lines.monthly == "quatar" and not lines.monthly == "monthly":
                ...
                query1 += " where t2.settle_reputation_date  BETWEEN '" + str(lines.fiscal_year.date_start) + "' AND '" + str(lines.fiscal_year.date_end) + "' AND t2.id is not null and t2.settle_type = 644"

                query += " where (t2.claim_date < '" + str(lines.fiscal_year.date_start) + "') and t2.id " \
                         " not in (select t2.id from policytransaction as t1 " \
                         " left join claim_policy as t2 on t2.claim_id =t1.id " \
                         " where ((t2.settle_date < '" + str(lines.fiscal_year.date_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
            ...
            if lines.fiscal_year and lines.monthly == "monthly" and not lines.monthly == "yearly" and not lines.monthly == "quatar":
                ...
                if lines.months == '01-01':
                    ...
                    query4 += " where t2.claim_date <= '" + str(
                        jan_end) + "' and t2.id  not in (select t2.id from policytransaction as t1 " \
                                   " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                   " where  (t2.settle_date <= '" + str(jan_end) + "' and " \
                                   " (t2.settle_type =650 or t2.settle_type =651)) or " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(jan_start) + "' AND '" + str(jan_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(jan_start) + "' AND '" + str(jan_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(jan_start) + "') and t2.id " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where ((t2.settle_date < '" + str(jan_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                ...
                if lines.months == '01-02':
                    ...
                    feb_end = str(end_year) + str('-02-28')
                    query4 += " where t2.claim_date <= '" + str(
                        feb_end) + "' and t2.id  not in (select t2.id from policytransaction as t1 " \
                                   " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                   " where  (t2.settle_date <= '" + str(feb_end) + "' and " \
                                   " (t2.settle_type =650 or t2.settle_type =651)) or " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(feb_start) + "' AND '" + str(feb_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(feb_start) + "' AND '" + str(feb_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(feb_start) + "') and t2.id " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where ((t2.settle_date < '" + str(feb_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                             " or  (t2.settle_reputation_date < '" + str(feb_start) + "' and (t2.settle_type =644 ))))"

                if lines.months == '01-03':
                    mar_start = str(end_year) + str('-03-01')
                    mar_end = str(end_year) + str('-03-31')
                    query4 += " where t2.claim_date <= '" + str(mar_end) + "' and t2.id  not in (select t2.id from policytransaction as t1 " \
                                    " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                    " where  (t2.settle_date <= '" + str(mar_end) + "' and " \
                                    " (t2.settle_type =650 or t2.settle_type =651)) or " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(mar_start) + "' AND '" + str(mar_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(mar_start) + "' AND '" + str(mar_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(mar_start) + "') and t2.id " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where ((t2.settle_date < '" + str(mar_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                ...
                if lines.months == '01-04':
                    ...
                    apr_end = str(start_year) + str('-04-30')
                    query4 += " where t2.claim_date <= '" + str(
                        apr_end) + "' and t2.id  not in (select t2.id from policytransaction as t1 " \
                                    " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                    " where  (t2.settle_date <= '" + str(apr_end) + "' and " \
                                    "(t2.settle_type =650 or t2.settle_type =651)) or " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(apr_start) + "' AND '" + str(apr_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(apr_start) + "' AND '" + str(apr_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(apr_start) + "') and t2.id " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where ((t2.settle_date < '" + str(apr_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                             " or  (t2.settle_reputation_date < '" + str(apr_start) + "' and (t2.settle_type =644 ))))"

                if lines.months == '01-05':
                    may_start = str(start_year) + str('-05-01')
                    may_end = str(start_year) + str('-05-31')
                    query4 += " where t2.claim_date <= '" + str(may_end) + "' and t2.id  " \
                              " not in (select t2.id from policytransaction as t1 " \
                              " left join claim_policy as t2 on t2.claim_id =t1.id " \
                              " where  (t2.settle_date <= '" + str(may_end) + "' and " \
                              " (t2.settle_type =650 or t2.settle_type =651)) or " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(may_start) + "' AND '" + str(may_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(may_start) + "' AND '" + str(may_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(may_start) + "') and t2.id " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where ((t2.settle_date < '" + str(may_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                             " or  (t2.settle_reputation_date < '" + str(may_start) + "' and (t2.settle_type =644 ))))"

                if lines.months == '01-06':
                    june_start = str(start_year) + str('-06-01')
                    june_end = str(start_year) + str('-06-30')
                    query4 += " where t2.claim_date <= '" + str(june_end) + "' and t2.id  " \
                                                                           " not in (select t2.id from policytransaction as t1 " \
                                                                           " left join claim_policy as t2 on t2.claim_id =t1.id " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(june_start) + "' AND '" + str(june_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(june_start) + "' AND '" + str(june_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(june_start) + "') and t2.id " \
                              " not in (select t2.id from policytransaction as t1 " \
                              " left join claim_policy as t2 on t2.claim_id =t1.id " \
                              " where ((t2.settle_date < '" + str(june_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                ...
                if lines.months == '01-07':
                    ...
                    query4 += " where t2.claim_date <= '" + str(
                        jul_end) + "' and t2.id  not in (select t2.id from policytransaction as t1 " \
                                   " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                   " where  (t2.settle_date <= '" + str(jul_end) + "' and " \
                                                                                   "(t2.settle_type =650 or t2.settle_type =651)) or " \
                                                                                   " (t2.settle_reputation_date <= '" + str(
                        jul_end) + "' and (t2.settle_type =644))) "
                    query3 += " where t2.claim_date  BETWEEN '" + str(jul_start) + "' AND '" + str(jul_end) + "' "
                    query2 += " where t2.settle_date  BETWEEN '" + str(jul_start) + "' AND '" + str(jul_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(jul_start) + "' AND '" + str(jul_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(jul_start) + "') and t2.id " \
                                                                             " not in (select t2.id from policytransaction as t1 " \
                                                                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                             " where ((t2.settle_date < '" + str(
                        jul_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                                      " or  (t2.settle_reputation_date < '" + str(
                        jul_start) + "' and (t2.settle_type =644 ))))"
                if lines.months == '01-08':
                    aug_start = str(start_year) + str('-08-01')
                    aug_end = str(start_year) + str('-08-31')
                    query4 += " where t2.claim_date <= '" + str(aug_end) + "' and t2.id  " \
                                                                            " not in (select t2.id from policytransaction as t1 " \
                                                                            " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                            " where  (t2.settle_date <= '" + str(
                        aug_end) + "' and " \
                                    " (t2.settle_type =650 or t2.settle_type =651)) or " \
                                    " (t2.settle_reputation_date <= '" + str(
                        aug_end) + "' and (t2.settle_type =644))) "
                    query3 += " where t2.claim_date  BETWEEN '" + str(aug_start) + "' AND '" + str(aug_end) + "' "
                    query2 += " where t2.settle_date  BETWEEN '" + str(aug_start) + "' AND '" + str(aug_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(aug_start) + "' AND '" + str(aug_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(aug_start) + "') and t2.id " \
                                                                            " not in (select t2.id from policytransaction as t1 " \
                                                                            " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                            " where ((t2.settle_date < '" + str(
                        aug_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                                     " or  (t2.settle_reputation_date < '" + str(
                        aug_start) + "' and (t2.settle_type =644 ))))"
                if lines.months == '01-09':
                    sep_start = str(start_year) + str('-09-01')
                    sep_end = str(start_year) + str('-09-30')
                    query4 += " where t2.claim_date <= '" + str(sep_end) + "' and t2.id  " \
                                                                           " not in (select t2.id from policytransaction as t1 " \
                                                                           " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                           " where  (t2.settle_date <= '" + str(
                        sep_end) + "' and " \
                                   " (t2.settle_type =650 or t2.settle_type =651)) or " \
                                   " (t2.settle_reputation_date <= '" + str(
                        sep_end) + "' and (t2.settle_type =644))) "
                    query3 += " where t2.claim_date  BETWEEN '" + str(sep_start) + "' AND '" + str(sep_end) + "' "
                    query2 += " where t2.settle_date  BETWEEN '" + str(sep_start) + "' AND '" + str(sep_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(sep_start) + "' AND '" + str(sep_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(sep_start) + "') and t2.id " \
                                                                            " not in (select t2.id from policytransaction as t1 " \
                                                                            " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                            " where ((t2.settle_date < '" + str(
                        sep_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                                     " or  (t2.settle_reputation_date < '" + str(
                        sep_start) + "' and (t2.settle_type =644 ))))"
                if lines.months == '01-10':
                    oct_start = str(start_year) + str('-10-01')
                    oct_end = str(start_year) + str('-10-31')
                    query4 += " where t2.claim_date <= '" + str(oct_end) + "' and t2.id  " \
                                                                           " not in (select t2.id from policytransaction as t1 " \
                                                                           " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                           " where  (t2.settle_date <= '" + str(
                        oct_end) + "' and " \
                                   " (t2.settle_type =650 or t2.settle_type =651)) or " \
                                   " (t2.settle_reputation_date <= '" + str(
                        oct_end) + "' and (t2.settle_type =644))) "
                    query3 += " where t2.claim_date  BETWEEN '" + str(oct_start) + "' AND '" + str(oct_end) + "' "
                    query2 += " where t2.settle_date  BETWEEN '" + str(oct_start) + "' AND '" + str(oct_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(oct_start) + "' AND '" + str(oct_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(oct_start) + "') and t2.id " \
                                                                            " not in (select t2.id from policytransaction as t1 " \
                                                                            " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                            " where ((t2.settle_date < '" + str(
                        oct_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                                     " or  (t2.settle_reputation_date < '" + str(
                        oct_start) + "' and (t2.settle_type =644 ))))"
                if lines.months == '01-11':
                    nov_start = str(start_year) + str('-11-01')
                    nov_end = str(start_year) + str('-11-30')
                    query4 += " where t2.claim_date <= '" + str(nov_end) + "' and t2.id  " \
                                                                           " not in (select t2.id from policytransaction as t1 " \
                                                                           " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                           " where  (t2.settle_date <= '" + str(
                        nov_end) + "' and " \
                                   " (t2.settle_type =650 or t2.settle_type =651)) or " \
                                   " (t2.settle_reputation_date <= '" + str(
                        nov_end) + "' and (t2.settle_type =644))) "
                    query3 += " where t2.claim_date  BETWEEN '" + str(nov_start) + "' AND '" + str(nov_end) + "' "
                    query2 += " where t2.settle_date  BETWEEN '" + str(nov_start) + "' AND '" + str(nov_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(nov_start) + "' AND '" + str(nov_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(nov_start) + "') and t2.id " \
                                                                            " not in (select t2.id from policytransaction as t1 " \
                                                                            " left join claim_policy as t2 on t2.claim_id =t1.id " \
                                                                            " where ((t2.settle_date < '" + str(
                        nov_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
                                     " or  (t2.settle_reputation_date < '" + str(
                        nov_start) + "' and (t2.settle_type =644 ))))"

                if lines.months == '01-12':
                    dec_start = str(start_year) + str('-12-01')
                    dec_end = str(start_year) + str('-12-31')
                    query4 += " where t2.claim_date <= '" + str(dec_end) + "' and t2.id  " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where  (t2.settle_date <= '" + str(dec_end) + "' and " \
                             " (t2.settle_type =650 or t2.settle_type =651)) or " \
                    ...
                    query2 += " where t2.settle_date  BETWEEN '" + str(dec_start) + "' AND '" + str(dec_end) + "' AND  t2.id is not null and (t2.settle_type = 651 or t2.settle_type = 650)"
                    query1 += " where t2.settle_reputation_date  BETWEEN '" + str(dec_start) + "' AND '" + str(dec_end) + "' AND  t2.id is not null and t2.settle_type = 644"
                    query += " where (t2.claim_date < '" + str(dec_start) + "') and t2.id " \
                             " not in (select t2.id from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where ((t2.settle_date < '" + str(dec_start) + "' and (t2.settle_type =650 or t2.settle_type =651))" \
            ...
            if lines.quarter and not lines.monthly == 'yearly' and not lines.monthly == 'monthly':
                if lines.quarter == "q2" or lines.quarter == "q3" or lines.quarter == "q4" or lines.quarter == "q1":
                    query4 += " where t2.claim_date <= '" + str(lines.date_to) + "' and t2.id  " \
                              " not in (select t2.id from policytransaction as t1 " \
                              " left join claim_policy as t2 on t2.claim_id =t1.id " \
                    ...
                    query += " where (t2.claim_date < '" + str(lines.date_from) + "') and t2.id " \
                              " not in (select t2.id from policytransaction as t1 " \
                              " left join claim_policy as t2 on t2.claim_id =t1.id " \
            ...
            if lines.filterby == 'false':
                ...
                query4 += "  and t16.is_company is " + str(lines.filterby) + " order by t1.group "
                query3 += "  and t16.is_company is " + str(lines.filterby) + " order by t1.group "
                query2 += " and t16.is_company is " + str(lines.filterby) + " order by t1.group "
                query1 += " and t16.is_company is " + str(lines.filterby) + " order by t1.group "
                query  += " and t16.is_company is " + str(lines.filterby) + " order by t1.group"

            if lines.filterby =='individual_group':
                query4 += " and t1.group = '" + str(
                    lines.group_name) + "' order by t1.group "
                query3 += " and t1.group = '" + str(
                    lines.group_name) + "' order by t1.group "
                query2 += " and t1.group= '" + str(
                    lines.group_name) + "' order by t1.group "
                query1 += "  and t1.group = '" + str(
                    lines.group_name) + "' order by t1.group "
                query += " and t1.group = '" + str(
                    lines.group_name) + "' order by t1.group"

            if lines.filterby =='internal_reference':
                query4 += "  and t1.partner_internal = '" + str(
                    lines.internal_reference) + "' order by t1.group "
                query3 += " and t1.partner_internal = '" + str(
                    lines.internal_reference) + "' order by t1.group "
                query2 += " and t1.partner_internal= '" + str(
                    lines.internal_reference) + "' order by t1.group "
                query1 += " and t1.partner_internal = '" + str(
                    lines.internal_reference) + "' order by t1.group "
                query += " and t1.partner_internal = '" + str(
                    lines.internal_reference) + "' order by t1.group"

            if lines.filterby =='individual_client':
                query4 += "  and t16.id = " + str(
                    lines.client_name.id) + " order by t1.group "
                query3 += " and t16.id = " + str(
                    lines.client_name.id) + " order by t1.group "
                query2 += " and t16.id = " + str(
                    lines.client_name.id) + " order by t1.group "
                query1 += " and t16.id = " + str(
                    lines.client_name.id) + " order by t1.group "
                query += "   and t16.id = " + str(
                    lines.client_name.id) + " order by t1.group"
            ...
            if lines.filterby == 'true':
                ...
                query4 += "  and t16.is_company is " + str(
                    lines.filterby) + " order by t1.group "
                query3 += " and t16.is_company is " + str(
                    lines.filterby) + " order by t1.group "
                query2 += "  and t16.is_company is " + str(
                    lines.filterby) + " order by t1.group "
                query1 += "  and t16.is_company is " + str(
                    lines.filterby) + " order by t1.group "
                query += " and t16.is_company is " + str(
                    lines.filterby) + " order by t1.group"

            # print(query4,"QUERYTS closing ")
            # print(query,"QUERYTS opening ")
        ...
        report_head = 'Claim Detail for the period of  ' + str(quat)
        report_head1 = 'Start Date : ' + str(
            lines.date_from) + '  to End Date : ' + str(lines.date_to)
        report_head2 = str(quat)
        report_head4 = 'IRDA-Claims-Detail'
        ...
        sheet.write(5, 1, ('Ticket No'), bold3)
        sheet.write(5, 2, ('Ticket Created On'), bold3)
        sheet.write(5, 3, ('Docket No'), bold3)
        sheet.write(5, 4, ('Claim No'), bold3)
        # sheet.write(10, 5, ('Claim No'), bold)
        sheet.write(5, 5, ('Insureds Name/Group'), bold)
        sheet.write(5, 6, ('Patients Name/Location'), bold)
        sheet.write(5, 7, ('Insurers Name/Concerned Person'), bold)
        sheet.write(5, 8, ('Policy Name/No'), bold)
        sheet.write(5, 9, ('Surveyor/TPA Name'), bold)
        sheet.write(5, 10, ('Brief Description'), bold)
        sheet.write(5, 11, ('Date of Loss/Intimation'), bold3)
        sheet.write(5, 12, ('Estimated Loss'), border)
        sheet.write(5, 13, ('Claimed Amount'), border)
        sheet.write(5, 14, ('Claim Status'), bold)
        sheet.write(5, 15, ('Claim Sub-status'), bold)
        sheet.write(5, 16, ('Settlement Type'), bold)
        sheet.write(5, 17, ('Settlement Amount'), border)
        sheet.write(5, 18, ('Date'), bold3)
        sheet.write(5, 19, ('Reply'), bold)
        sheet.write(6, 1, ('Claims pending at the beginning of the period (' + str(lines.fiscal_year.date_start) + ')'), bold)
        ...
        # sheet.merge_range('S3:T3', report_head4, bold1)
        sheet.merge_range('S1:T1', report_head7, bold1)
        sheet.merge_range('B3:U3', report_head, bold2)
        sheet.merge_range('B4:U4', report_head1, bold_less)
        # sheet.merge_range('B8:U8', report_head2, bold2)
        sheet.merge_range('B2:F2', report_head6, bold_less)
...
GeneralXlsx('report.clickbima.irda_claims_detail.xlsx', 'irdaclaimsdetail.report')
