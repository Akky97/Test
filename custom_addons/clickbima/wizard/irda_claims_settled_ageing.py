...
class Idrasettledreport(models.TransientModel):
    _name = "irdasettled.report"
    _description = "IRDA Claims Settled Ageing Report"
    ...
    def generate_xlsx_report(self):
        ...
        return self.env['report'].get_action(self, report_name='clickbima.irda_claims_settled.xlsx', data=data)
...
class GeneralXlsx(ReportXlsx):
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        with registry.cursor() as cr:
            ...
            if lines.fiscal_year and lines.monthly == "yearly" and not lines.monthly == "quatar":
                query += " select DATE_PART('day','" + str(lines.fiscal_year.date_end) + "'::timestamp -  t2.claim_date::timestamp)," \
                    ...
                    " where t2.settle_date  between  '" + str(lines.fiscal_year.date_start) + "' and  '" + str(lines.fiscal_year.date_end) + "'  " \
                    " AND t2.id is not null  and (t2.settle_type =650 or t2.settle_type =651)) as tab1 group by tab1.days "
            ...
            if lines.quarter and not lines.monthly == 'yearly':
                if lines.quarter == "q2" or lines.quarter == "q3" or lines.quarter == "q4" or lines.quarter == "q1":
                    query += "select DATE_PART('day','" + str(lines.date_to) + "'::timestamp -  t2.claim_date::timestamp)," \
                             ...
                             " from policytransaction as t1 " \
                             " left join claim_policy as t2 on t2.claim_id =t1.id " \
                             " where t2.settle_date  between  '" + str(lines.date_from) + "' and  '" + str(lines.date_to) + "' AND t2.id is not null  and (t2.settle_type =650 or t2.settle_type =651)) as tab1 group by tab1.days "
        ...
        report_head = 'Ageing of Settled claims (From Policy Holders And Customers)'
        report_head1 = 'for Financial Year ' + str(lines.fiscal_year.name)
        report_head2 = str(quat)
        report_head3 = 'Total'
        report_head4 = 'IRDA-Claims-Settled'
        ...
        sheet.write(10, 1, ('Settled claims ageing buckets'), bold)
        sheet.write(10, 2, ('Number Of Claims'), bold3)
        sheet.write(10, 3, ('Claims Amount'), bold3)
        sheet.write(11, 1, ('Settled for upto 1 month'),border1)
        sheet.write(12, 1, ('Settled for greater than 1 month and upto 3 months'), border1)
        sheet.write(13, 1, ('Settled for greater than 3 month and upto 6 months'), border1)
        sheet.write(14, 1, ('Settled for greater than 6 month and upto 12 months'), border1)
        sheet.write(15, 1, ('Settled for more than 1 year'), border1)
        sheet.write(16, 1, ('Total Settled Up to ' + str(lines.fiscal_year.date_end)), bold)
        ...
        for res in temp:
            ...
            if res['days'] ==30:
                ...
                sheet.write(11, 3, res['amount'], border2)
            if res['days'] == 90:
                total_count += res['count']
                total_amount += amount
...
GeneralXlsx('report.clickbima.irda_claims_settled.xlsx', 'irdasettled.report')
