...
class OdooAPI(http.Controller):
    ...
    def product_template_search(self, **params):
        ...
        try:
            ...
            search = ''
            if "filter" in params:
                if "rating" == params['filter']:
                    search = 'rating_count DESC'
                if "new" == params['filter']:
                    search = 'create_date DESC'
                if "top" == params['filter']:
                    search = 'sale_count_pando DESC'
                if "lth" == params['filter']:
                    search = 'list_price ASC'
                if "htl" == params['filter']:
                    search = 'list_price DESC'

            record = request.env[model].sudo().search(domain, order=search)
            base_url = request.env['ir.config_parameter'].sudo().search([('key', '=', 'web.base.url')], limit=1)
            warehouse = request.env['stock.warehouse'].sudo().search(
                [('company_id', '=', website.company_id.id)], limit=1)
